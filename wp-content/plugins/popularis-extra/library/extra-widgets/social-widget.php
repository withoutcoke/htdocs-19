...
if (!class_exists('Popularis_Extra_Social_Widget')) {

    class Popularis_Extra_Social_Widget extends WP_Widget {
        ...
        public function __construct() {
            ...
            $this->social_services_array = apply_filters('popularis_social_widget_profiles', array(
                'twitter' => array(
                    'name' => 'Twitter',
                    'url' => '',
                ),
            ...
            parent::__construct(
                    'popularis_social',
                    esc_html__('Popularis: Social Icons', 'popularis-extra'),
                    array(
                        'classname' => 'widget-populariswp-social social-widget',
                        'description' => esc_html__('Display your social media icons.', 'popularis-extra'),
        ...
        public function social_widget_style() {
            ?>
            <style>
                .populariswp-social-widget-services-list { padding-top: 10px; }
                .populariswp-social-widget-services-list li { cursor: move; background: #fafafa; padding: 10px; border: 1px solid #e5e5e5; margin-bottom: 10px; }
                .populariswp-social-widget-services-list li p { margin: 0 }
                .populariswp-social-widget-services-list li label { margin-bottom: 3px; display: block; color: #222; }
                .populariswp-social-widget-services-list li label span.fa { margin-right: 10px }
                .populariswp-social-widget-services-list .placeholder { border: 1px dashed #e3e3e3 }
                .populariswp-widget-select { width: 100% }
            ...
            <script>
                (function ($) {
                    $(document).ajaxSuccess(function (e, xhr, settings) {
                        var widget_id_base = 'popularis_social';
                        if (settings.data.search('action=save-widget') != -1 && settings.data.search('id_base=' + widget_id_base) != -1) {
                            PopulariswpSortServices();
                        }
                    });

                    function PopulariswpSortServices() {
                        $('.populariswp-social-widget-services-list').each(function () {
                            var id = $(this).attr('id');
                            $('#' + id).sortable({
        ...
        public function widget($args, $instance) {
            ...
            ?>
            <ul class="populariswp-social-icons <?php echo esc_attr($transition); ?> style-<?php echo esc_attr($style); ?>">
                <?php
                // Original Array
                ...
                foreach ($social_services as $key => $val) {
                    ...
                    if ($link) {
                        ...
                        echo '<li class="populariswp-' . esc_attr($key) . '">';
        ...
        public function form($instance) {
            ...
            $instance = wp_parse_args((array) $instance, array(
                'title' => esc_attr__('Follow Us', 'popularis-extra'),
                'style' => esc_html__('Light', 'popularis-extra'),
                'transition' => esc_html__('Rotate', 'popularis-extra'),
                'font_size' => '',
                'border_radius' => '',
                'bg_color' => '',
                'bg_hover_color' => '',
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('style')); ?>"><?php esc_html_e('Style:', 'popularis-extra'); ?></label>
                <select class='widefat' name="<?php echo esc_attr($this->get_field_name('style')); ?>" id="<?php echo esc_attr($this->get_field_id('style')); ?>">
                    <option value="light" <?php selected($instance['style'], 'light') ?>><?php esc_html_e('Light', 'popularis-extra'); ?></option>
                    <option value="dark" <?php selected($instance['style'], 'dark') ?>><?php esc_html_e('Dark', 'popularis-extra'); ?></option>
                    <option value="colored" <?php selected($instance['style'], 'colored') ?>><?php esc_html_e('Colored', 'popularis-extra'); ?></option>
                    <option value="simple" <?php selected($instance['style'], 'simple') ?>><?php esc_html_e('Simple', 'popularis-extra'); ?></option>
                </select>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('transition')); ?>"><?php esc_html_e('Transition:', 'popularis-extra'); ?></label>
                <select class='widefat' name="<?php echo esc_attr($this->get_field_name('transition')); ?>" id="<?php echo esc_attr($this->get_field_id('transition')); ?>">
                    <option value="no-transition" <?php selected($instance['transition'], 'no-transition') ?>><?php esc_html_e('None', 'popularis-extra'); ?></option>
                    <option value="float" <?php selected($instance['transition'], 'float') ?>><?php esc_html_e('Float', 'popularis-extra'); ?></option>
                    <option value="rotate" <?php selected($instance['transition'], 'rotate') ?>><?php esc_html_e('Rotate', 'popularis-extra'); ?></option>
                    <option value="zoomout" <?php selected($instance['transition'], 'zoomout') ?>><?php esc_html_e('Zoom Out', 'popularis-extra'); ?></option>
                </select>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('target')); ?>"><?php esc_html_e('Link Target', 'popularis-extra'); ?>:</label>
                <br />
                <select class="populariswp-widget-select" name="<?php echo esc_attr($this->get_field_name('target')); ?>" id="<?php echo esc_attr($this->get_field_id('target')); ?>">
                    <option value="blank" <?php selected($instance['target'], 'blank') ?>><?php esc_html_e('Blank', 'popularis-extra'); ?></option>
                    <option value="self" <?php selected($instance['target'], 'self') ?>><?php esc_html_e('Self', 'popularis-extra'); ?></option>
                </select>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('size')); ?>"><?php esc_html_e('Dimensions', 'popularis-extra'); ?>:</label>
                ...
                <small><?php esc_html_e('Example:', 'popularis-extra'); ?> 40px</small>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('font_size')); ?>"><?php esc_html_e('Font Size', 'popularis-extra'); ?>:</label>
                ...
                <small><?php esc_html_e('Example:', 'popularis-extra'); ?> 18px</small>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('border_radius')); ?>"><?php esc_html_e('Border Radius', 'popularis-extra'); ?></label>
                ...
                <small><?php esc_html_e('Example:', 'popularis-extra'); ?> 4px</small>
            </p>

            <p>
                <label class="color-label" for="<?php echo esc_attr($this->get_field_id('bg_color')); ?>"><?php esc_html_e('Background Color', 'popularis-extra'); ?></label>
            ...
            <p>
                <label class="color-label" for="<?php echo esc_attr($this->get_field_id('bg_hover_color')); ?>"><?php esc_html_e('Background Color Hover', 'popularis-extra'); ?></label>
            ...
            <p>
                <label class="color-label" for="<?php echo esc_attr($this->get_field_id('color')); ?>"><?php esc_html_e('Color', 'popularis-extra'); ?></label>
            ...
            <p>
                <label class="color-label" for="<?php echo esc_attr($this->get_field_id('color_hover')); ?>"><?php esc_html_e('Color Hover', 'popularis-extra'); ?></label>
            ...
            <p>
                <label class="color-label" for="<?php echo esc_attr($this->get_field_id('border_color')); ?>"><?php esc_html_e('Border Color', 'popularis-extra'); ?></label>
            ...
            <p>
                <label class="color-label" for="<?php echo esc_attr($this->get_field_id('border_hover_color')); ?>"><?php esc_html_e('Border Color Hover', 'popularis-extra'); ?></label>
            ...
            ?>
            <h3 style="margin-top:20px;margin-bottom:0;"><?php esc_html_e('Social Links', 'popularis-extra'); ?></h3>
            <ul id="<?php echo esc_attr($field_id_services); ?>" class="populariswp-social-widget-services-list">
                <input type="hidden" id="<?php echo esc_attr($field_name_services); ?>" value="<?php echo esc_attr($field_name_services); ?>">
                <input type="hidden" id="<?php echo esc_attr(wp_create_nonce('populariswp_fontawesome_social_widget_nonce')); ?>">
                <?php
                // Social array
                $social_services_array = $this->social_services_array;
        ...
        public function colors($args, $instance) {
            ...
            <?php
            if ($bg_color || $bg_hover_color || $color || $color_hover || $border_color || $border_hover_color) :
                ?>
                <style>
                    #<?php echo $id; ?>.widget-populariswp-social ul li a { 
                        <?php
                        if ($bg_color) {
                    ...
                    #<?php echo $id; ?>.widget-populariswp-social ul li a:hover { 
                        <?php
                        if ($bg_hover_color) {
                            echo 'background-color:' . $bg_hover_color;
                        }
                        ?>;
                        <?php
                        if ($color_hover) {
                        ...
                        if ($border_hover_color) {
                            echo 'border-color:' . $border_hover_color . '!important';
                        }
                        ?>;
                    }
                </style>
