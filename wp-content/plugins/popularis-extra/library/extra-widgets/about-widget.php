...
// Start class
if ( ! class_exists( 'Popularis_Extra_About_Me_Widget' ) ) {
	class Popularis_Extra_About_Me_Widget extends WP_Widget {
		...
		public function __construct() {
			...
			$this->social_services_array = apply_filters( 'popularis_about_me_widget_profiles', array(
				'facebook'		=> array(
					'name'		=> 'Facebook',
					'url'		=> ''
				),
			...
			parent::__construct(
				'popularis_about_me',
				esc_html__( 'Popularis: About Me', 'popularis-extra' ),
				array(
					'classname'   => 'widget-populariswp-about-me about-me-widget',
					'description' => esc_html__( 'Adds a about me widget.', 'popularis-extra' ),
		...
		public function social_widget_style() { ?>
			<style>
				.populariswp-about-me-widget-services-list { padding-top: 10px; }
				.populariswp-about-me-widget-services-list li { cursor: move; background: #fafafa; padding: 10px; border: 1px solid #e5e5e5; margin-bottom: 10px; }
				.populariswp-about-me-widget-services-list li p { margin: 0 }
				.populariswp-about-me-widget-services-list li label { margin-bottom: 3px; display: block; color: #222; }
				.populariswp-about-me-widget-services-list li label span.fa { margin-right: 10px }
				.populariswp-about-me-widget-services-list .placeholder { border: 1px dashed #e3e3e3 }
			...
			<script>
				( function( $ ){
					$(document).ajaxSuccess(function(e, xhr, settings) {
						var widget_id_base = 'popularis_about_me';
					...
					function PopulariswpSortServices() {
						$( '.populariswp-about-me-widget-services-list' ).each( function() {
							var id = $(this).attr( 'id' );
							$( '#'+ id ).sortable( {
			...
			echo $args['before_widget'];
				...
				<div class="populariswp-about-me">
					...
					<div class="populariswp-about-me-avatar clr">
						...
						// Display the name
						if ( $name ) :?>
							<h3 class="populariswp-about-me-name"><?php echo esc_html( $name ); ?></h3>
						<?php endif; ?>
					...
					// Display the text
					if ( $text ) : ?>
						<div class="populariswp-about-me-text clr"><?php echo do_shortcode( $text ); ?></div>
					<?php endif;

					// Display the social
					if ( $social_services ) : ?>

						<ul class="populariswp-about-me-social style-<?php echo esc_attr( $social_style ); ?>">
							<?php
							// Original Array
		...
		public function form( $instance ) {
			$instance = wp_parse_args( ( array ) $instance, array(
				'title'       		=> esc_html__( 'About Me', 'popularis-extra' ),
				'avatar' 			=> '',
				'name'  			=> esc_html__( 'John Doe', 'popularis-extra' ),
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'avatar' ) ); ?>"><?php esc_html_e( 'Image URL', 'popularis-extra' ); ?>:</label> 
				...
				<input class="populariswp-widget-upload-button button button-secondary" type="button" value="<?php esc_html_e( 'Upload Image', 'popularis-extra' ); ?>" />
			</p>

			<p>
			    <label for="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>"><?php esc_html_e('Name:', 'popularis-extra') ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>"><?php esc_html_e( 'Description:','popularis-extra' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id('social_style') ); ?>"><?php esc_html_e('Social Style:', 'popularis-extra'); ?></label>
				<br />
				<select class='widget-select widefat' name="<?php echo esc_attr( $this->get_field_name('social_style') ); ?>" id="<?php echo esc_attr( $this->get_field_id('social_style') ); ?>">
					<option value="color" <?php selected( $instance['social_style'], 'color' ) ?>><?php esc_html_e( 'Color', 'popularis-extra' ); ?></option>
					<option value="light" <?php selected( $instance['social_style'], 'light' ) ?>><?php esc_html_e( 'Light', 'popularis-extra' ); ?></option>
					<option value="dark" <?php selected( $instance['social_style'], 'dark' ) ?>><?php esc_html_e( 'Dark', 'popularis-extra' ); ?></option>
				</select>
			</p>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id('target') ); ?>"><?php esc_html_e( 'Social Link Target:', 'popularis-extra' ); ?></label>
				<br />
			...
			<ul id="<?php echo esc_attr( $field_id_services ); ?>" class="populariswp-about-me-widget-services-list">
			...
			<script type="text/javascript">
				(function($) {
					...
					$( document ).ready( function() {
						...
						$( '.populariswp-widget-upload-button' ).click(function(e) {
