...
namespace WPDataAccess\List_Table {
	...
	class WPDA_List_Table_Menu extends WPDA_List_Table {
			...
			if ( 'table_rows' === $column_name ) {
				if ( 'InnoDB' === $item['engine'] ) {
					...
					if ( $item[ $column_name ] < $innodb_count ) {
						...
						$wpdadb = WPDADB::get_db_connection( $this->schema_name );
						$suppress = $wpdadb->suppress_errors();
						$norows   = $this->count_rows( $item['table_name'] );
		...
		protected function process_bulk_action_edit_db() {
			...
			if ( ! WPDADB::upd_remote_database( $database, $host, $user, $passwd, $port, $schema, true, $database_old ) ) {
				$msg = new WPDA_Message_Box(
					[
		...
		protected function process_bulk_action_create_db() {
			...
			if ( isset( $_REQUEST['database_location'] ) && 'local' === $_REQUEST['database_location'] ) {
				...
				if ( ! WPDADB::add_remote_database( $database, $host, $user, $passwd, $port, $schema ) ) {
					$msg = new WPDA_Message_Box(
						[
		...
		protected function process_bulk_action_settings_table() {
			if ( isset( $_REQUEST['settings_table_name'] ) && isset( $_REQUEST['settings'] ) ) {
				...
				$settings            = sanitize_text_field( wp_unslash( $_REQUEST['settings'] ) ); // input var okay.
				...
				<div class="wpda_upload">
					<form method="post" action="?page=<?php echo $this->page; ?>" onsubmit="return editdb_validate_form();">
						...
						<div>
							...
							<div style="height:10px;"></div>
							<label for="edit_remote_host" style="vertical-align:baseline;" class="database_item_label">MySQL
								host:</label>
							<input type="text" name="edit_remote_host" id="edit_remote_host" maxlength="64" value="<?php echo esc_attr( $host ); ?>">
							<br/>
							<label for="edit_remote_user" style="vertical-align:baseline;" class="database_item_label">MySQL
								username:</label>
							<input type="text" name="edit_remote_user" id="edit_remote_user" value="<?php echo esc_attr( $user ); ?>">
							<br/>
							<label for="edit_remote_passwd" style="vertical-align:baseline;" class="database_item_label">MySQL
								password:</label>
							<input type="text" name="edit_remote_passwd" id="edit_remote_passwd" value="<?php echo esc_attr( $passwd ); ?>">
							<br/>
							<label for="edit_remote_port" style="vertical-align:baseline;" class="database_item_label">MySQL
								port:</label>
							<input type="text" name="edit_remote_port" id="edit_remote_port" value="3306" value="<?php echo esc_attr( $port ); ?>">
							<br/>
							<label for="edit_remote_schema" style="vertical-align:baseline;" class="database_item_label">MySQL
								schema:</label>
