...
namespace WPDataAccess\CSV_Files {
	...
	class WPDA_CSV_Mapping {
		...
		public function show() {
			...
			$table_list = WPDA_Dictionary_Lists::get_tables( false, $this->schema_name );
			...
			<script type="text/javascript">
				...
				function wpda_get_columns(init=false) {
					var schema_name = '<?php echo esc_attr( $this->schema_name ); ?>';
			...
			<a href="javascript:void(0)"
			   onclick="if (confirm('<?php echo __( "Ones started, a CSV import cannot be interrupted.\\n\\nDo you want to continue?", 'wp-data-access' ); ?>')) { jQuery('#import_form').submit(); }"
			   class="button"
			>
