...
if ( is_admin() ) {
	...
	if (is_plugin_active('leaflet-maps-marker-pro/leaflet-maps-marker.php') || (is_plugin_active('maps-marker-pro/maps-marker-pro.php') || class_exists('Maps_Marker_Pro')) ) {
		if (!is_multisite()) {
			exit('[Leaflet Maps Marker - activation failed]: Please deactivate "Maps Marker Pro" first before downgrading to (outdated) "Leaflet Maps Marker" (<a href="https://www.mapsmarker.com/v3.12.3" target="_blank">click here for more details</a>)');
		} else {
			if (is_network_admin()) {
				echo 'Network wide activation of the (outdated) plugin "Leaflet Maps Marker" failed as the plugin "Maps Marker Pro" is still active on subsites. Please activate "Leaflet Maps Marker" on desired subsites only!<br/><a href="https://www.mapsmarker.com/v3.12.3" target="_blank">Click here for more details</a>';
			} else {
				echo 'Too bad you want to use the (outdated) free version again :-( Please deactivate "Maps Marker Pro" first before downgrading to "Leaflet Maps Marker" (<a href="https://www.mapsmarker.com/v3.12.3" target="_blank">click here for more details</a>)!';
			}
		}
	}
}
...
class Leafletmapsmarker
{
	...
	function lmm_plugins_page_add_links($links, $file) {
		...
		if ($file == $plugin) {
			...
			$translation_link = '<a href="https://translate.mapsmarker.com/" target="_blank" title="' . esc_attr__('translations','lmm') . '"><img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-translations.png" width="16" height="16" alt="translations"></a>';
	...
	function lmm_update_pointer_footer_script() {
		...
		$lmm_version_new = get_option( 'leafletmapsmarker_version' );
		$version_without_dots = "lmmv" . str_replace('.', '', $lmm_version_new);
		$pointer_content = '<h3>' . sprintf(__('Important notice for<br/>Leaflet Maps Marker','lmm'), $lmm_version_new) . '</h3>';
		...
		$pointer_content .= '<p><div style="background:#f99755;display:block;padding:25px;text-decoration:none;color:#2702c6;">' . __('Leaflet Maps Marker has earned its retirement - existing users get a parting gift: a 50% coupon code for an unexpiring<br/>Maps Marker Pro license (<a href="https://demo.mapsmarker.com" target="_blank">view demo</a>).<br/>For a limited time only, get yours now!<p><strong><a href="https://www.mapsmarker.com/v3.12.3" target="_blank">Read more & get 50% coupon code</a></strong>') . '</p></div></p>';
		//$pointer_content .= '<hr noshade size="1"/><p><a style="display:block;padding:5px;text-decoration:none;color:#2702c6;" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_pro_upgrade">' . __('Upgrade to pro version for even more features - click here to find out how you can start a free 30-day-trial easily','lmm') . '</a></p>';
	  ?>
	...
	function lmm_dashboard_widget(){
		...
		echo '<p style="background:#f99755;display:block;padding:5px;text-decoration:none;color:#2702c6;text-align:center;" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_pro_upgrade">' . __('Leaflet Maps Marker has earned its retirement - existing users get a parting gift: a 50% coupon code for an unexpiring Maps Marker Pro license (<a href="https://demo.mapsmarker.com" target="_blank">view demo</a>).<br/><strong>For a limited time only, <a href="https://www.mapsmarker.com/v3.12.3" target="_blank">get yours now</a>!</strong>','lmm') . '</p><hr style="border:0;height:1px;background-color:#d8d8d8;"/></p>';
		if ($result != NULL) {
		...
		if ($show_rss == 1)	{
				...
				<div style="display:inline-block;"><a href="https://translate.mapsmarker.com/" target="_blank"><img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-translations.png" width="16" height="16" alt="translations"> ' . __('translations','lmm') . '</a></div>&nbsp;
	...
	function lmm_admin_menu() {
		...
		if ( !empty($lmm_options) ) { //info: needed to suppress warning when reseting settings
			$page2 = add_submenu_page('leafletmapsmarker_markers', 'Leaflet Maps Marker - ' . __('List all markers', 'lmm'), '<img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-menu-list' . $mp6_icon . '.png"> ' . __('List all markers', 'lmm'), $lmm_options[ 'capabilities_edit' ], 'leafletmapsmarker_markers', array(&$this, 'lmm_list_markers') );
			$page3 = add_submenu_page('leafletmapsmarker_markers', 'Leaflet Maps Marker - ' . __('add/edit marker', 'lmm'), '<img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-menu-add' . $mp6_icon . '.png"> ' . __('Add new marker', 'lmm'), $lmm_options[ 'capabilities_edit' ], 'leafletmapsmarker_marker', array(&$this, 'lmm_marker') );
			$page4 = add_submenu_page('leafletmapsmarker_markers', 'Leaflet Maps Marker - ' . __('List all layers', 'lmm'), '<img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-menu-list' . $mp6_icon . '.png"> ' . __('List all layers', 'lmm'), $lmm_options[ 'capabilities_edit' ], 'leafletmapsmarker_layers', array(&$this, 'lmm_list_layers') );
			$page5 = add_submenu_page('leafletmapsmarker_markers', 'Leaflet Maps Marker - ' . __('add/edit layer', 'lmm'), '<img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-menu-add' . $mp6_icon . '.png"> ' . __('Add new layer', 'lmm'), $lmm_options[ 'capabilities_edit' ], 'leafletmapsmarker_layer', array(&$this, 'lmm_layer') );
			$page8 = add_submenu_page('leafletmapsmarker_markers', 'Leaflet Maps Marker - ' . __('Support', 'lmm'), '<img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-menu-help' . $mp6_icon . '.png"> ' . __('Support', 'lmm'), $lmm_options[ 'capabilities_edit' ], 'leafletmapsmarker_help', array(&$this, 'lmm_help') );
		} else {
			$page = '';
			$page2 = '';
			$page3 = '';
		...
		if ( !empty($lmm_options) ) { //info: needed to suppress warning when reseting settings
			$page3b = add_submenu_page('leafletmapsmarker_markers', 'Leaflet Maps Marker - ' . __('Import/Export', 'lmm'), '<hr noshade size="1" style="margin-top:0px;"/><img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-menu-import-export' . $mp6_icon . '.png"> ' . __('Import/Export', 'lmm'), $lmm_options[ 'capabilities_edit' ], 'leafletmapsmarker_import_export', array(&$this, 'lmm_import_export') );
		} else {
			$page3b = '';
		}
		$page6 = add_submenu_page('leafletmapsmarker_markers', 'Leaflet Maps Marker - ' . __('Tools', 'lmm'), '<img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-menu-tools' . $mp6_icon . '.png"> ' . __('Tools', 'lmm'), 'activate_plugins','leafletmapsmarker_tools', array(&$this, 'lmm_tools') );
		$page7 = add_submenu_page('leafletmapsmarker_markers', 'Leaflet Maps Marker - ' . __('Settings', 'lmm'), '<img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-menu-settings' . $mp6_icon . '.png"> ' . __('Settings', 'lmm'), 'activate_plugins', 'leafletmapsmarker_settings', array(&$this, 'lmm_settings') );
		$page11 = add_submenu_page('leafletmapsmarker_markers', 'Leaflet Maps Marker - Maps Marker API', '<img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-menu-api' . $mp6_icon . '.png"> ' . __('Maps Marker APIs', 'lmm'), 'activate_plugins', 'leafletmapsmarker_apis', array(&$this, 'lmm_apis') );
		$page12 = add_submenu_page('leafletmapsmarker_markers', 'Leaflet Maps Marker - Retirement info / coupon code', '<div style="background:#FFFF00;color:#000;padding:3px;line-height:1.8em;"><img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-up.png"> ' . __('Retirement info<br/>&nbsp;&nbsp;&nbsp;&amp; coupon code', 'lmm') . '&nbsp;&nbsp;&nbsp;</div>', $lmm_options[ 'capabilities_edit' ], 'leafletmapsmarker_retirement', array(&$this, 'lmm_retirement') );
		if ( !empty($lmm_options) ) { //info: needed to suppress warning when reseting settings
			$page10 = add_submenu_page('leafletmapsmarker_markers', 'Leaflet Maps Marker - ' . __('Upgrade to Pro', 'lmm'), '<div style="background:#F99755;color:#000;padding:3px;line-height:1.8em;"><img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-up.png"> ' . __('Upgrade to Pro', 'lmm') . '&nbsp;&nbsp;&nbsp;</div>', $lmm_options[ 'capabilities_edit' ], 'leafletmapsmarker_pro_upgrade', array(&$this, 'lmm_pro_upgrade') );
		} else {
			$page10 = '';
		}
		...
		if ( $lmm_options[ 'admin_bar_integration' ] == 'enabled' && current_user_can($lmm_options[ 'capabilities_edit' ]) )
		{
			...
			$menu_items = array_merge($menu_items, array(
					array(
						...
						'parent' => 'lmm',
						'title' => '<span style="background:#FFFF00;color:#000;padding:3px;text-shadow:none;"><img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-up.png"> ' . __('Retirement info & coupon code','lmm') . '&nbsp;&nbsp;</span>',
							'href' => LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_retirement'
					),
					array(
						'id' => 'lmm-upgrade',
						'parent' => 'lmm',
						'title' => '<span style="background:#F99755;color:#000;padding:3px;text-shadow:none;"><img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-up.png"> ' . __('Upgrade to Maps Marker Pro','lmm') . '&nbsp;&nbsp;</span>',
	...
	function lmm_install_and_updates() {
		...
		$current_version = "v3123"; //2do - mandatory: change on each update to new version!
	...
	function lmm_plugin_meta_links() {
		...
		function leafletmapsmarker_filter_plugin_meta($links, $file) {
			if ( $file == FB_BASENAME ) {
				array_unshift(
					$links,
					'<a href="https://www.mapsmarker.com/v3.12.3" target="_blank">'.__('Retirement info/coupon code','lmm').'</a>',
					'<br/><a href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_pro_upgrade">'.__('Upgrade to Maps Marker Pro','lmm').'</a>'
				);
			}
			return $links;
		}
