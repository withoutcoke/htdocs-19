...
class Menu_Tools extends Menu {
	...
	public function import() {
		...
		$assign_mode = (isset($_POST['assign_mode']) && in_array($_POST['assign_mode'], array('on', 'missing', 'off'))) ? $_POST['assign_mode'] : 'off';
		$assign_map = (isset($_POST['assign_map'])) ? $db->get_map($_POST['assign_map']) : null;
				...
				for ($i = 1; $i <= $markers_batches; $i++) {
					...
					if ($result) {
						...
						foreach ($batch as $marker) {
							if ($assign_mode === 'on' || ($assign_mode === 'missing' && !count($marker['maps']))) {
								$assigns[$id] = $assign_map->id;
	...
	protected function show() {
		...
		$overlays = $mmp_settings->get_overlays();
		...
		<div class="wrap mmp-wrap">
							...
							<div id="trackContent" class="mmp-map-batch-settings-group">
								...
								<div class="mmp-map-batch-setting">
									...
									<div class="mmp-map-setting-input">
										...
										<img class="mmp-gpx-start-icon mmp-align-middle" src="<?= (!$settings['gpxStartIcon']) ? plugins_url('images/leaflet/gpx-start.png', __DIR__) : MMP::$icons_url . $settings['gpxStartIcon'] ?>" />
									</div>
								</div>
								<div class="mmp-map-batch-setting">
									<div class="mmp-map-setting-desc">
								...
								<div class="mmp-map-batch-setting">
									...
									<div class="mmp-map-setting-input">
										...
										<img class="mmp-gpx-end-icon mmp-align-middle" src="<?= (!$settings['gpxEndIcon']) ? plugins_url('images/leaflet/gpx-end.png', __DIR__) : MMP::$icons_url . $settings['gpxEndIcon'] ?>" />
									</div>
								</div>
								<div class="mmp-map-batch-setting mmp-advanced">
									<div class="mmp-map-setting-desc">
				...
				<div id="import_markers" class="mmp-tools-section">
					...
					<div>
						...
						<form id="import_form" method="POST">
							...
							<?= esc_html__('Assign to map', 'mmp') ?>:<br />
							<select id="assign_map" name="assign_map">
								<?php foreach ($maps as $map): ?>
									<option value="<?= $map->id ?>">[<?= $map->id ?>] <?= esc_html($map->name) ?> (<?= $map->markers ?> <?= esc_html__('markers', 'mmp') ?>)</option>
								<?php endforeach; ?>
							</select><br />
							<label><input name="assign_mode" type="radio" value="off" checked="checked" /> <?= esc_html__('Off (markers will only be assigned according to their assignment info)', 'mmp') ?></label><br />
							<label><input name="assign_mode" type="radio" value="missing" /> <?= esc_html__('Missing (markers with missing assignment info will be assigned to this map)', 'mmp') ?></label><br />
							<label><input name="assign_mode" type="radio" value="on" /> <?= esc_html__('On (all markers will be assigned to this map, even if they have assignment info)', 'mmp') ?></label><br />
			...
			<div id="iconsGpx" class="mmp-admin-modal">
				<div class="mmp-admin-modal-content">
					...
					<div class="mmp-admin-modal-body">
						<div id="iconsListGpx">
							<img class="mmp-icon" src="<?= plugins_url('images/leaflet/marker.png', __DIR__) ?>" data-icon="" />
							<img class="mmp-icon" src="<?= plugins_url('images/leaflet/gpx-start.png', __DIR__) ?>" data-icon="" />
							<img class="mmp-icon" src="<?= plugins_url('images/leaflet/gpx-end.png', __DIR__) ?>" data-icon="" />
							<?php foreach ($upload->get_icons() as $icon): ?>
